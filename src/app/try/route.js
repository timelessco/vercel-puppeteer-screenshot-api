...
async function takeScreenshot(urlStr, fullPage) {
  let browser = null;
  ...
  try {
    ...
    if (!screenshot) {
      throw new Error(`Failed to capture screenshot: ${lastError?.message || 'Unknown error'}`);
    }
...
export async function GET(request) {
  ...
  for (let retry = 1; retry <= maxRetries; retry++) {
    try {
      console.log(`API attempt ${retry}/${maxRetries} for URL: ${urlStr}`);

      if (retry > 1) {
        console.log(`Waiting 5 seconds before retry ${retry}...`);
        await new Promise(resolve => setTimeout(resolve, 2000));
      }
      
      const screenshot = await takeScreenshot(urlStr, fullPage);

      console.log(`API attempt ${retry} succeeded`);
      ...
      lastError = err;
      console.error(`API attempt ${retry}/${maxRetries} failed:`, err.message);
      
      if (retry < maxRetries) {
        const delay = Math.min(1000 * Math.pow(2, retry - 1), 5000); // Exponential backoff, max 5s
        console.log(`Waiting ${delay}ms before retry ${retry + 1}...`);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
    }
  }

  console.error(`All ${maxRetries} API attempts failed. Final error:`, lastError?.message);
